...
// adds a yaml style 'frontmatter' --- header with a timestamp of the update time.
...
pub fn archive(original_file: &String, backup_file: &String, new_lines: &Vec<String>, name: &str) {
    ...
    let frontmatter_header: String = String::from(format!(
        "---\ntitle: \"TODO {}, {} {}\"\ndate: \"{}\"\ntags: [\"TODO-{}\"]\n---",
        &name, &datestamp, &timestamp, &datestamp, &name
    ));
    appended_lines.push(frontmatter_header);
    ...
    if Path::new(&original_file).exists() {
        ...
        fs::copy(&original_file, &backup_file)
            .expect(&format!("failed making a backup of {}", &original_file).to_string());
        ...
        let original_lines = fs::read_to_string(&original_file)
            .expect(&format!("Unable to read contents of {}", &original_file).to_string());

        let mut in_header_section = false;
        let header_start_strings = vec![
            "---",
            "id:",
            "title:",
            "desc:",
        ...
        for mut line in original_lines.split("\n") {
            ...
            if line.starts_with("---") {
                // toggle true/false
                in_header_section = if in_header_section == false {
                    true
                } else {
                    false
                };
    ...
    for line in appended_lines {
        writeln!(file, "{}", line)
            .expect(&format!("Unable to overwrite {}", &original_file).to_string());
    }
}
